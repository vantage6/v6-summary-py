...
image:
	@echo "Building ${REGISTRY}/algorithm/summary:${TAG}-v6-${VANTAGE6_VERSION}"
	@echo "Building ${REGISTRY}/algorithm/summary:latest"
	docker buildx build \
		--tag ${REGISTRY}/algorithm/summary:${TAG}-v6-${VANTAGE6_VERSION} \
		--tag ${REGISTRY}/algorithm/summary:${TAG} \
		--platform ${PLATFORMS} \
		-f ./Dockerfile \
